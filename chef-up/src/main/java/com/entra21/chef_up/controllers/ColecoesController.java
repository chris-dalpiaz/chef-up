...
import com.entra21.chef_up.entities.Colecoes;
import com.entra21.chef_up.repository.ReceitaColecaoRepository;
import com.entra21.chef_up.repository.ColecoesRepository;
...
public class ColecoesController {
    private final ColecoesRepository colecoesRepository;
    private final ReceitaColecaoRepository receitaColecaoRepository;

    public ColecoesController(ColecoesRepository colecoesRepository, ReceitaColecaoRepository receitaColecaoRepository) {
    ...
    public Colecoes buscarColecao(@PathVariable Integer idColecao) {
     ...
     */
    @PostMapping
    public Colecoes criarColecao(@RequestBody Colecoes colecao) {
    ...
    public Colecoes alterarColecao(
            @PathVariable Integer idColecao,
            @RequestBody Colecoes colecao
    ) {
        Colecoes alterar = colecoesRepository.findById(idColecao)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Coleção não encontrada"));
    ...
    public Colecoes removerColecao(@PathVariable Integer idColecao) {
        Colecoes colecao = colecoesRepository.findById(idColecao)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Coleção não encontrada"));
